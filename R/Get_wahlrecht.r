...
Institute <- RunSQL( 'select * from Institute where autoupdate = 1;')
...
Institute[, LINK := paste0(urlbase,urlsub,urlext)]
...
# for (INo in 1:1) {
  ...
  HTML <- getURL( Institute[INo,"LINK"]
  ...
  umfragen[, 1] = as.Date( umfragen$Datum, "%d.%m.%Y" ) 
...
setwd('/data/git/R/DEU-Umfragen/bash/')
